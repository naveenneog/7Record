...
- Added `BackgroundJobRegistry` (`SevenRecord.Infrastructure.Jobs`), which makes shutdown own every long-running background job. Export, offline transcription, edited-preview rendering and project post-processing are now tracked by name, each with a cancellation token linked to its caller's. Closing the window cancels them all and waits — but the wait is **bounded**, because an app you cannot quit is a worse bug than a leaked worker, and a job that outlives the drain is recorded by name rather than hidden. 12 tests, including a job that deliberately ignores its token.
- Created `SevenRecord.App.Presentation`, the non-UI assembly the charter's tenth architecture boundary has been reserving since M0. It holds `AudioHealthNarrator` (audio drift/discontinuity/missing-audio warning thresholds), `RecorderTextFormatter` (project display names, component failure messages), `RecorderStatusFormatter` (mix descriptions, worst-of capture state, project root naming) and `BestEffortCleanup` (silent removal of scratch files and partial renders). All were previously private statics inside a 3,800-line page, so nothing could reach them and no test could prove a threshold was right; they now have 39 tests. `RecorderTextFormatter.IssueMessage` deliberately takes plain component/message pairs rather than `WindowsRecordingIssue`, so the assembly does not drag Win2D and the Windows App SDK into anything that tests it.
- Moved caption loading into `SevenRecord.Editor.CaptionSessionLoader`, where the rest of the project's caption persistence already lives.
...
- **Closing the app could leave an FFmpeg or Whisper worker running.** Export, transcription, edited-preview rendering and post-processing were all started fire-and-forget with no cancellation token and no record of them anywhere — `MediaWorkerExportClient` already terminated its worker on cancellation and `LocalWhisperTranscriber` already accepted a token, but `MainPage` passed neither. Shutdown neither cancelled nor awaited any of them, so a worker could outlive the app and keep writing to a file the user believed was finished. All four now run through the job registry, and `ShutdownCoreAsync` drains it before approving the close.
- **A second Record click could kill the app mid-startup.** `OnStartRecordingClicked` read the recorder state, then awaited display selection, capture-target picking, readiness refresh and camera-studio shutdown, and only then called `RecorderStateMachine.BeginStart()` — which throws when the state is not `Idle`. A second click or the Ctrl+Shift+R global hotkey arriving during those awaits also saw `Idle`, and the loser threw `InvalidOperationException` on the UI thread out of an `async void` handler, terminating the process along with any recording in progress. The recorder is now claimed atomically through a new non-throwing `TryBeginStart` *before* any await, so the losing command is refused instead of crashing. `TryAbortStart` releases a claim whose preconditions failed, returning straight to `Idle` rather than travelling through `Stopping` and telling the user a recording that never started is being torn down.
- Released the recorder claim when preparing to record *throws* rather than merely declining. Without that the fault barrier would contain the exception but leave the recorder wedged in `Starting`, refusing every later command.
- Removed a null-forgiving operator from the recording start path: `PrepareToRecordAsync` now returns the validated capture target instead of a bool, so the non-null guarantee is carried by the type rather than by a comment. This also cleared a `CS8604` warning the restructure exposed.
- Made partial-render cleanup safe for a path with no directory component; the original used a null-forgiving operator and would have thrown.
